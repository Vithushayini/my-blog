...
export default function Contact() {
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-lg">
        <div className="flex items-center mb-4">
            ...
            <input onChange={handleInput} type="text" name="name"  id="name" value={inputs.name??""}  className="border rounded px-2 py-1 w-3/4" />
        </div>
        <div className="flex items-center mb-4">
            <label htmlFor="email" className="w-1/4">Email:</label>
            <input onChange={handleInput} type="email" name="email"   id="email" value={inputs.email??""}  className="border rounded px-2 py-1 w-3/4" />
        </div>
        <div className="flex items-center mb-4">
            <label htmlFor="message" className="w-1/4">Message:</label>
            <textarea onChange={handleInput} id="message" value={inputs.message??""} name="message"  className="border rounded px-2 py-1 w-3/4" rows="4"></textarea>
        </div>
